...
async function main() {
  ...
  if (!apiKey || apiKey.trim() === "") {
    console.error(
      "OPENAI_API_KEY is not set. Set it in the process environment; this harness does not read .env files."
    );
    process.exitCode = 1;
    return;
  }
  ...
  if (!model) {
    fail("Missing required model. Usage: npm run verify:openai:planner -- --model <model-name>");
  }
  ...
  if (!evidencePack) {
    fail("Could not locate Evidence Pack path in OpenAI planner CLI output.");
  }

  await verifyEvidencePack(evidencePack, model, apiKey);

  console.log("OpenAI planner local verification passed.");
  console.log("");
  console.log(`Model: ${model}`);
  console.log(`Evidence Pack: ${evidencePack}`);
  console.log("Verified:");
  console.log("- required evidence files are present");
  console.log("- provider and model metadata match the selected OpenAI planner");
  console.log("- expected final report sections are present");
  console.log("- API key does not appear in recorded evidence");
}
...
async function assertBuiltCliExists() {
  try {
    ...
    fail("dist/cli.js was not found. Run npm run build before npm run verify:openai:planner.");
  }
}
...
async function verifyEvidencePack(evidencePack, model, apiKey) {
  ...
  assertEqual(task.planner_provider, "openai", "task.json planner_provider");
  assertEqual(task.planner_model, model, "task.json planner_model");
  assertEqual(plan.provider, "openai", "plan.json provider");
...
function fail(message) {
  console.error(`OpenAI planner local verification failed: ${message}`);
  process.exit(1);
}
